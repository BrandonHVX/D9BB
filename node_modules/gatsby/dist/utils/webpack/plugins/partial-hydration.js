...
class PartialHydrationPlugin {
  ...
  _generateManifest(compilation, rootContext) {
    ...
    const json = {};
    const mapOriginalModuleToPotentiallyConcatanetedModule = new Map(); // @see https://github.com/facebook/react/blob/3f70e68cea8d2ed0f53d35420105ae20e22ce428/packages/react-server-dom-webpack/src/ReactFlightWebpackPlugin.js#L220-L252
  ...
  async addClientModuleEntries( // @ts-ignore
            ...
            if (connection.dependency) {
              if (connection.originModule instanceof _webpack.NormalModule) {
                if (connection.originModule.resource.includes(`async-requires`)) {
                  asyncRequires = connection.originModule; // Remove page template "import" from async-requires.
  ...
  addEntry(compilation, context, entry, options)
  /* Promise<module> */
  {
  ...
  apply(compiler) {
    ...
    compiler.hooks.thisCompilation.tap(this.name, (compilation, {
      ...
      normalModuleFactory.hooks.parser.for(`javascript/dynamic`).tap(this.name, parserCallback); // add dangling css modules to the app entry
        ...
        for (const cssModule of modulesToInsertIntoApp.sort((a, b) => compilation.moduleGraph.getPostOrderIndex(a) - compilation.moduleGraph.getPostOrderIndex(b))) {
          ...
          for (const group of appChunk.groupsIterable) {
            if (!group.getModulePostOrderIndex(cssModule)) {
              group.setModulePostOrderIndex(cssModule, // @ts-ignore
      ...
      compilation.hooks.processAssets.tap({
        ...
        compilation.emitAsset(emitManifestPath, new _webpack.default.sources.RawSource(JSON.stringify({ ...this._previousManifest,
          ...manifest
        }, null, 2), false));
      });
    });
