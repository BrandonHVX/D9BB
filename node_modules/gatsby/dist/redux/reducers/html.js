    ...
    case `DELETE_CACHE`:
      {
        if (action.cacheIsCorrupt) {
          ...
          state.trackedHtmlFiles.forEach(htmlFile => {
            htmlFile.isDeleted = true; // there was a change somewhere, so just in case we mark those files are dirty as well
    ...
    case `DELETE_PAGE`:
      {
        ...
        htmlFile.isDeleted = true; // TBD if dirtiness need to change
    ...
    case `SET_SSR_WEBPACK_COMPILATION_HASH`:
      {
        if (state.ssrCompilationHash !== action.payload) {
          state.ssrCompilationHash = action.payload; // we will mark every html file as dirty, so we can safely reset
